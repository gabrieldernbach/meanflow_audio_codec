"""
Benchmark script to compare JAX audio.py implementation with torch flowmo implementation.
...
# Add flowmo to path to import torch implementation
...
try:
    ...
    print(f"Warning: Could not import torch implementation: {e}")
    TORCH_AVAILABLE = False


def benchmark_jax_pipeline(
    data_dir: str,
    batch_size: int = 32,
    ...
    """Benchmark the JAX audio.py pipeline."""
    print(f"\n{'='*70}")
    print("Benchmarking JAX Implementation (build_audio_pipeline)")
    print(f"{'='*70}")
    ...
    for batch in pipeline:
        ...
        # batch shape is (batch_size, frame_sz, n_channels)
        total_samples += batch.shape[0] * batch.shape[1]  # batch_size * frame_sz
        batch_count += 1
...
def main():
    ...
    print(f"  Prefetch (JAX only): {prefetch}")
    ...
    if TORCH_AVAILABLE:
        ...
        print("\nSkipping torch benchmark (torch not available)")
    ...
    if torch_results:
        ...
        print("Comparison:")
        print("=" * 70)
        print(f"{'Metric':<30} {'JAX':<20} {'Torch':<20} {'Ratio':<10}")
        print("-" * 70)
        
        batches_ratio = (
            jax_results["batches_per_second"] / torch_results["batches_per_second"]
            if torch_results["batches_per_second"] > 0
            else 0
        )
        samples_ratio = (
            jax_results["samples_per_second"] / torch_results["samples_per_second"]
            if torch_results["samples_per_second"] > 0
            else 0
        )
        time_ratio = (
            torch_results["elapsed_time"] / jax_results["elapsed_time"]
            if jax_results["elapsed_time"] > 0
            else 0
        )
        first_batch_ratio = (
            jax_results["first_batch_time"] / torch_results["first_batch_time"]
            if torch_results["first_batch_time"] > 0
            else 0
        )
        
        print(
            f"{'Batches/sec':<30} "
            f"{jax_results['batches_per_second']:<20.2f} "
            f"{torch_results['batches_per_second']:<20.2f} "
            f"{batches_ratio:<10.2f}x"
        )
        print(
            f"{'Samples/sec':<30} "
            f"{jax_results['samples_per_second']:<20,.0f} "
            f"{torch_results['samples_per_second']:<20,.0f} "
            f"{samples_ratio:<10.2f}x"
        )
        print(
            f"{'Time (seconds)':<30} "
            f"{jax_results['elapsed_time']:<20.2f} "
            f"{torch_results['elapsed_time']:<20.2f} "
            f"{time_ratio:<10.2f}x"
        )
        print(
            f"{'First batch time (s)':<30} "
            f"{jax_results['first_batch_time']:<20.3f} "
            f"{torch_results['first_batch_time']:<20.3f} "
            f"{first_batch_ratio:<10.2f}x"
        )
        
        print("\n" + "-" * 70)
        if batches_ratio > 1.0:
            print(f"✓ JAX implementation is {batches_ratio:.2f}x faster (batches/sec)")
        elif batches_ratio < 1.0:
            print(f"✓ Torch implementation is {1.0/batches_ratio:.2f}x faster (batches/sec)")
        else:
            print("✓ Both implementations have similar performance")
        
        if samples_ratio > 1.0:
            print(f"✓ JAX implementation is {samples_ratio:.2f}x faster (samples/sec)")
        elif samples_ratio < 1.0:
            print(f"✓ Torch implementation is {1.0/samples_ratio:.2f}x faster (samples/sec)")
        
        print("=" * 70)
    else:
        print("\n" + "=" * 70)
        print("JAX Results Summary:")
        print("=" * 70)
        print(f"  Batches/sec: {jax_results['batches_per_second']:.2f}")
        print(f"  Samples/sec: {jax_results['samples_per_second']:,.0f}")
        print(f"  First batch time: {jax_results['first_batch_time']:.3f}s")
        print("=" * 70)
